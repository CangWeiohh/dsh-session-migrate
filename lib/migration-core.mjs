...
export async function executeOffline(rawPlan, { dryRun = false, skipStopCheck = false } = {}) {
  ...
  try {
    ...
    if (projectionExisted) {
      ...
      if (projection.tables?.sessions) {
        for (const node of preview.nodes) {
          const record = projection.tables.sessions[node.id]
          if (record) {
            projection.tables.sessions[node.id] = { ...record, identity: { ...record.identity, cwd: preview.targetCwd } }
          }
        }
    ...
    const cleanupCommand = `${quoteForShell(preview.plan.nodeExecutable)} ${quoteForShell(preview.plan.cliFile)} cleanup --backup ${quoteForShell(backupDir)} --yes`
    return { ok: true, dryRun: false, backupDir, cleanupCommand, summary, warnings: recall.warnings, reloadRequired: true }
  } catch (error) {
...
function assertBackupContained(backupDir, harnessRoot) {
  const root = path.resolve(harnessRoot, '.session-migrate-backups')
  const absolute = path.resolve(backupDir)
  if (absolute === root || !absolute.startsWith(`${root}${path.sep}`)) {
    throw new MigrationError('backup directory is outside this Harness migration backup root', 'INVALID_BACKUP_PATH')
  }
  return absolute
}

export async function inspectCleanup(backupDir) {
  const requested = path.resolve(backupDir)
  const manifest = await readJson(path.join(requested, 'manifest.json'))
  if (manifest?.version !== TX_VERSION || !manifest?.plan?.harnessRoot) {
    throw new MigrationError('backup manifest is not a recognized migration transaction', 'INVALID_BACKUP_MANIFEST')
  }
  const absolute = assertBackupContained(requested, manifest.plan.harnessRoot)
  if (manifest.phase !== 'COMMITTED') {
    throw new MigrationError(`only COMMITTED backups may be cleaned (current phase: ${String(manifest.phase)})`, 'BACKUP_NOT_COMMITTED')
  }
  if (await exists(path.join(absolute, 'rollback.json'))) {
    throw new MigrationError('a rolled-back transaction backup may not be cleaned by this command', 'BACKUP_ROLLED_BACK')
  }
  const size = await directorySize(absolute)
  return {
    backupDir: absolute,
    sessionId: manifest.plan.sessionId,
    sessionIds: (manifest.nodes ?? []).map((node) => node.id),
    createdAt: manifest.committedAt ?? null,
    ...size,
  }
}

export async function cleanupOffline(backupDir, { confirm = false, skipStopCheck = false } = {}) {
  const preview = await inspectCleanup(backupDir)
  if (!confirm) return { ok: true, dryRun: true, cleanup: preview, confirmationRequired: true }
  if (!skipStopCheck) assertDshStopped(path.resolve(backupDir, '..', '..'))
  await fsp.rm(preview.backupDir, { recursive: true, force: false, maxRetries: 2 })
  return { ok: true, dryRun: false, cleaned: preview }
}
...
async function resolveDecoder(sessionModuleUrl) {
  if (sessionModuleUrl) {
    try { return await loadDecoder({ sessionModuleUrl }) } catch {}
  }
  for (const candidate of [
    'file:///Applications/DSH%20Desktop.app/Contents/Resources/app/node_modules/@deepseek-ai/dsh-session/lib/index.js',
  ]) {
    try { return await loadDecoder({ sessionModuleUrl: candidate }) } catch {}
  }
  throw new MigrationError('cannot locate the DSH session decoder; pass sessionModuleUrl explicitly', 'SESSION_DECODER_UNAVAILABLE')
}

export async function fixProjectionCache(harnessRoot, sessionId, { skipStopCheck = false, sessionModuleUrl = '' } = {}) {
  if (!skipStopCheck) assertDshStopped(harnessRoot)
  const plan = normalizePlan({ sessionId, targetWorkspaceId: 'fix-only', harnessRoot })
  const catalog = await sessionCatalog(plan.harnessRoot)
  const row = catalog.find((r) => String(r.header.id) === sessionId)
  if (!row) throw new MigrationError(`session not found: ${sessionId}`, 'SESSION_NOT_FOUND')
  const decoder = await resolveDecoder(sessionModuleUrl)
  const physical = decodePhysicalLog(row.buffer, row.filename)
  const events = decodeStorageEvents(physical.bodyText, decoder)
  const titleEvents = events.filter((e) => e.type === 'session/title')
  const lastTitle = titleEvents[titleEvents.length - 1]
  const projectionFile = path.join(plan.harnessRoot, 'storages', 'session_projcache.json')
  const projectionExisted = await exists(projectionFile)
  let projection = projectionExisted ? await readJson(projectionFile) : { unit: { name: 'session_projcache', version: 3 }, global: null, tables: { sessions: {} } }
  if (projection?.unit?.name !== 'session_projcache' || projection.unit.version !== 3) throw new MigrationError('only projection cache v3 is supported', 'UNSUPPORTED_PROJECTION_VERSION')
  if (!projection.tables?.sessions) projection.tables = { sessions: {} }
  const identity = { createdAt: physical.header.createdAt, ...(physical.header.cwd ? { cwd: physical.header.cwd } : {}) }
  const existing = projection.tables.sessions[sessionId]
  const before = existing ? { identity: existing.identity, titleVal: existing.rows?.title?.val ?? null } : null
  const rows = { ...(existing?.rows ?? {}) }
  if (lastTitle) rows.title = { ver: 1, seq: lastTitle.seq, val: lastTitle.data.title }
  projection.tables.sessions[sessionId] = { identity, rows }
  await atomicJson(projectionFile, projection)
  return {
    ok: true, sessionId, projectionFile,
    headerCwd: physical.header.cwd,
    title: lastTitle ? lastTitle.data.title : null,
    titleSource: lastTitle ? lastTitle.data.source.kind : null,
    titleSeq: lastTitle ? lastTitle.seq : null,
