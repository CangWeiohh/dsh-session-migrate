...
export async function inspectPlan(rawPlan, { allowAlreadyInTarget = false, allowExistingTarget = false } = {}) {
  ...
  const matches = catalog.filter((row) => row.header.id === plan.sessionId)
  if (matches.length !== 1) throw new MigrationError(`expected one session artifact, found ${matches.length}`, matches.length ? 'DUPLICATE_SESSION_ARTIFACTS' : 'SESSION_NOT_FOUND')
  const source = matches[0]
  if (!source.header.cwd) throw new MigrationError('session header has no cwd', 'SESSION_CWD_MISSING')
  if (source.projectKey !== projectKey(source.header.cwd)) throw new MigrationError('session header/path identity mismatch', 'SESSION_PATH_MISMATCH')
  if (!allowAlreadyInTarget && (await fsp.realpath(targetCwd)) === await fsp.realpath(source.header.cwd).catch(() => source.header.cwd)) throw new MigrationError('already in target workspace', 'ALREADY_IN_TARGET')
  if (source.header.parentSession) throw new MigrationError(`subagent session has parent ${source.header.parentSession}; migrate the tree together`, 'RELATED_SESSIONS')
  const children = catalog.filter((row) => row.header.parentSession === plan.sessionId).map((row) => row.header.id)
  if (children.length) throw new MigrationError(`session has child sessions: ${children.join(', ')}`, 'RELATED_SESSIONS', { children })
  const targetDir = path.join(plan.harnessRoot, 'sessions', projectKey(targetCwd), encodeSegment(plan.sessionId))
  if (!allowExistingTarget && await exists(targetDir)) throw new MigrationError('target session directory exists', 'TARGET_EXISTS')
  const decoder = await loadDecoder(plan)
  const beforePhysical = decodePhysicalLog(source.buffer, source.filename)
  const beforeEvents = decodeStorageEvents(beforePhysical.bodyText, decoder)
  const sourceWorkspaceIds = Object.entries(workspaceRows).filter(([, row]) => row.sessionIds?.includes(plan.sessionId)).map(([id]) => id)
  if (sourceWorkspaceIds.length > 1) throw new MigrationError('session belongs to multiple workspaces', 'MULTIPLE_WORKSPACES')
  return {
    plan, workspaceFile, projectionFile, workspace, workspaceRows, target, targetCwd, catalog, source,
    targetDir, targetLogPath: path.join(targetDir, source.filename), sourceWorkspaceId: sourceWorkspaceIds[0] ?? null,
    decoder, beforeEvents, beforeEventsHash: hashJson(beforeEvents), sourceHash: hashBuffer(source.buffer),
  }
}
...
export function assertDshStopped(harnessRoot, { stabilityMs = 1200 } = {}) {
  ...
  const suspicious = processes.split('\n').filter((line) => /DSH Desktop\.app|@deepseek-ai\/dsh\/lib\/bin\.js|dsh\/lib\/bin\.js/.test(line))
...
async function restore(backupDir) {
  ...
  await fsp.rm(manifest.targetDir, { recursive: true, force: true })
  await fsp.rm(manifest.sourceDir, { recursive: true, force: true })
  const quarantined = path.join(backupDir, 'quarantine-session')
  if (await exists(quarantined)) await fsp.rename(quarantined, manifest.sourceDir)
  else await copyDir(path.join(backupDir, 'session-original'), manifest.sourceDir)
...
export async function executeOffline(rawPlan, { dryRun = false, skipStopCheck = false } = {}) {
  ...
  if (!skipStopCheck) {
    ...
    await stableFingerprint([preview.source.logPath, preview.workspaceFile, (await exists(preview.projectionFile)) ? preview.projectionFile : null], delay)
    // Reinspect after the quiescence proof.
    Object.assign(preview, await inspectPlan(rawPlan))
  }
  const summary = { sessionId: preview.plan.sessionId, sourceCwd: preview.source.header.cwd, targetCwd: preview.targetCwd, sourceWorkspaceId: preview.sourceWorkspaceId, targetWorkspaceId: preview.plan.targetWorkspaceId, sourceLogPath: preview.source.logPath, targetLogPath: preview.targetLogPath, eventCount: preview.beforeEvents.length, eventHash: preview.beforeEventsHash }
  ...
  await copyDir(preview.source.sessionDir, path.join(backupDir, 'session-original'))
  ...
  const manifest = { version: TX_VERSION, phase: 'BACKED_UP', plan: preview.plan, sourceDir: preview.source.sessionDir, targetDir: preview.targetDir, workspaceFile: preview.workspaceFile, projectionFile: preview.projectionFile, projectionExisted, sourceHash: preview.sourceHash, recallCreated: [] }
  await atomicJson(path.join(backupDir, 'manifest.json'), manifest)
  try {
    const rewritten = rewriteSessionLogBuffer(preview.source.buffer, preview.source.filename, { sessionId: preview.plan.sessionId, expectedCwd: preview.source.header.cwd, targetCwd: preview.targetCwd })
    const staging = `${preview.targetDir}.staging-${process.pid}-${crypto.randomBytes(5).toString('hex')}`
    await fsp.mkdir(path.dirname(preview.targetDir), { recursive: true })
    await copyDir(preview.source.sessionDir, staging)
    await atomicWrite(path.join(staging, preview.source.filename), rewritten.buffer)
    const stagedDecoded = decodePhysicalLog(await fsp.readFile(path.join(staging, preview.source.filename)), preview.source.filename)
    const stagedEvents = decodeStorageEvents(stagedDecoded.bodyText, preview.decoder)
    if (stagedDecoded.header.cwd !== preview.targetCwd || hashJson(stagedEvents) !== preview.beforeEventsHash) throw new MigrationError('staged log semantic verification failed', 'STAGING_VERIFY_FAILED')
    await fsp.rename(staging, preview.targetDir)
    manifest.phase = 'TARGET_PUBLISHED'; await atomicJson(path.join(backupDir, 'manifest.json'), manifest)
    ...
    if (projectionExisted) {
      ...
      if (projection.tables?.sessions) delete projection.tables.sessions[preview.plan.sessionId]
    ...
    await fsp.rename(preview.source.sessionDir, path.join(backupDir, 'quarantine-session'))
    manifest.phase = 'SOURCE_QUARANTINED'; await atomicJson(path.join(backupDir, 'manifest.json'), manifest)
    ...
    const final = await inspectPlan({ ...rawPlan, targetWorkspaceId: preview.plan.targetWorkspaceId }, { allowAlreadyInTarget: true, allowExistingTarget: true }).catch((error) => { throw new MigrationError(`post-migration inspection failed: ${String(error)}`, 'POST_VERIFY_FAILED') })
    if (final.source.header.cwd !== preview.targetCwd || final.beforeEventsHash !== preview.beforeEventsHash) throw new MigrationError('post-migration content mismatch', 'POST_VERIFY_FAILED')
